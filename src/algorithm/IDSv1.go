...
func central(c *colly.Collector, wg *sync.WaitGroup) {
	...
	if start == destination {
		...
		for {
			...
			wg.Add(1)
			dls(path_of_url, start, iterasi, wg)
			wg.Wait()
		}
	}
}

func dls(path_of_url []string, url_scraped string, iterasi int, wg *sync.WaitGroup) {
	defer wg.Done()
	// fmt.Println("IN")
	...
	validasiLinkIDS(&link_to_visit, &list_of_url)
	// fmt.Println("OUT")
	iterasi -= 1
	for j := 0; j < len(list_of_url); j++ {
		...
		if list_of_url[j] == destination {
			...
			if iterasi > 0 {
				wg.Add(1)
				go func() {
					dls(new_path_of_url, list_of_url[j], iterasi, wg)
				}()
			}
		}
	}
